...
public:
	...
	void create_gui(void)
	{
		...
		add_member_control(this, "draw my quad", draw_my_quad);
	...
	bool init(cgv::render::context& ctx)
	{
		...
		if (draw_my_quad)
		{
			std::cout << "[DEBUG] Init draw cube, draw_my_quad:" << draw_my_quad << std::endl;
		...
		//   locations of the vertex layout
			...
			cgv::render::type_descriptor
				...
				material_type =
				cgv::render::element_descriptor_traits<cgv::media::illum::surface_material>
				::get_type_descriptor(vertices_cube[0].material);
			...
			success = vertex_array_cube.set_attribute_array(
				...
				vertices_cube.size(), // number of texcoord elements in the array
				sizeof(vertex_cube) // stride from one element to next
			) && success;
		...
		{
			std::cout << "[DEBUG] Init draw deafault, draw_my_quad:" << draw_my_quad << std::endl;
			// Init geometry buffers
		// - get a reference to the default shader, from which we're going to query named
		//   locations of the vertex layout
	...
	{
		std::cout << "[DEBUG] Now in draw_my_unit_sqaure..." << std::endl;
		//vertex_array_cube.enable(ctx);
		//glDrawArrays(GL_TRIANGLE_STRIP, 0, (GLsizei)vertices.size());
		fractal->use_vertex_array(&vertex_array_cube, vertices_cube.size(), GL_QUADS);
		fractal->draw_recursive(ctx, cgv::rgb(1.0f), 5, 0);
		//ctx.tesselate_unit_cube();
		//vertex_array_cube.disable(ctx);
		std::cout << "[DEBUG] Leaving draw_my_unit_sqaure !" << std::endl;
	}
};

// Create an instance of the demo class at plugin load and register it with the framework
//cgv::base::object_registration<cgv_demo> cgv_demo_registration("cgv_demo");
