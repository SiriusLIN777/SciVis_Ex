...
)
{
	...
	//std::cout << "[INFO] Fractal va_num_verts: " << va_num_verts << std::endl;
}
...
)
{
	...
	if (va)
	{
		...
		va->enable(ctx);
		//std::cout << "[DEBUG] Fractal va enable succeed." << std::endl;
		glDrawArrays(va_primitive_type, 0, (GLsizei)va_num_verts);
		va->disable(ctx);
		//std::cout << "[DEBUG] Fractal va disable succeed." << std::endl;
	}
	else
